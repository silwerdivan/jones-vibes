...
Your Goal: Complete the tasks in `fast-food-plan.md`.

Context:
1. Read `fast-food-activity.md` to see what happened last.
2. Read `fast-food-plan.md` to pick the next unchecked task.

Instructions:
1. Pick the SINGLE highest priority unchecked task.
2. Implement it.
3. Run verification (e.g., `npm test; npm run build`).
5. If verified, mark [x] in `fast-food-plan.md` and append a summary to `fast-food-activity.md`.
