...
- Historical note only: GitHub issue `#2` for the turn summary `HAPPINESS` label / stale sanity total is fixed as of commit `0bf6cec` on 2026-03-19 and the issue is closed. Older slice logs that mention `HAPPINESS 0` remain valid audit evidence for the pre-fix build, not the current baseline.
- That older summary regression was stronger than label drift alone. The persisted Week 3 summary tracked `sanityChange: +15`, but the pre-fix modal still displayed `HAPPINESS 0`.
- The same pre-fix summary bug affected negative sanity weeks too. The persisted Week 5 summary tracked `sanityChange: -10`, but the rendered modal still displayed `HAPPINESS 0`.
...
- The Week 8 summary bug was slightly different but still real on the pre-fix build. The visible line items already totaled `-5` sanity (`Ambient Stress -10`, `Cycle Recovery +5`), the hidden Life screen confirmed the checkpoint closed at `30` sanity, and the modal still printed `HAPPINESS 0`.
