...
If `Runner Context` includes resolved known issues, treat them as the starting baseline for the slice and only reopen them if the live app clearly regresses.
...
- GitHub issue `#2` is already fixed and closed. The turn summary no longer uses the legacy `HAPPINESS` label or the stale `summary-happiness-total` id.
- Treat older audit notes or slice files that mention `HAPPINESS 0` in the end-of-week summary as historical evidence from before commit `0bf6cec`, not as an open blocker to rediscover.
- If the live UI still contradicts that fix during this run, log it as a regression against the resolved baseline rather than as the original unresolved issue.
