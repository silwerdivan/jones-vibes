...
- **Encoding & File Integrity:** All file operations (read/write/replace) MUST strictly adhere to **UTF-8 encoding**. You MUST NOT introduce null bytes (`\x00`), Byte Order Marks (BOM), or mixed encodings (e.g., UTF-16) into any file. When using special characters, emojis, or symbols, ensure they are correctly handled as UTF-8. If a file appears corrupted or uses an incorrect encoding, you must restore it to clean UTF-8.
