...
- **Identified Elements:** City travel cards still need targeted DOM interaction because they are custom divs. Labor Sector job application is now fixed as baseline behavior under GitHub issue `#10`: scroll the targeted `Apply` button into view, click it directly, and verify `CURRENT SHIFT` or persisted `careerLevel` before continuing.
- **Next Action:** Start Week 9 from the authoritative Week 8 summary state, test whether `Work Shift x3` remains viable from `₡458 / Hunger 60% / Sanity 25 / Trauma Reboot 266h`, and keep treating issues `#7`, `#8`, `#9`, and `#10` as fixed baseline behavior unless a fresh slice reproduces a one-choice opener, stale helper export, false `live_continuity` report on visible onboarding, or a failed Labor `Apply` after the new scroll-and-verify sequence.
